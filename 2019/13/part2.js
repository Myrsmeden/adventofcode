...
fs.readFile('intcodes.txt', 'utf8', (_, data) => {
  ...
  let input = 0
  while (!amp.halted) {
    const x = amp.run(input)
    const y = amp.run(input)
    const id = amp.run(input)
    if (!amp.halted) {
      paddleX = id === 3 ? x : paddleX
      ballX = id === 4 ? x : ballX
      points = x === -1 && y === 0 ? id : points
    }
    input = ballX && paddleX ? Math.max(-1, Math.min(ballX - paddleX, 1)) : 0
    console.log(input)
  }
  console.log(points)
  return
